...
<div class="w-full max-w-md bg-slate-900 p-8 rounded-xl border border-slate-800">
    ...
    <form class="space-y-4" action="{{ route('register') }}" method="POST">
        ...
        <div>
            ...
            <input type="text" name="name" class="w-full bg-slate-950 border border-slate-700 rounded-lg p-2 text-white" required>
        </div>
        <div>
            <label class="block text-sm mb-1">Email</label>
            <input type="email" name="email" class="w-full bg-slate-950 border border-slate-700 rounded-lg p-2 text-white" required>
        </div>
        <div>
