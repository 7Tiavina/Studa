...
<body class="bg-background text-on-background font-sans antialiased" x-data="{ activeTab: 'dashboard', showMessenger: false, openChats: [] }">
            ...
            <div class="w-80 bg-white rounded-t-xl shadow-2xl flex flex-col overflow-hidden text-slate-800 pointer-events-auto border border-slate-200">
                ...
                <div class="bg-white p-3 flex justify-between items-center border-b border-gray-100 cursor-pointer shadow-sm" @click="chat.minimized = !chat.minimized">
                    ...
                    <div class="flex items-center gap-1">
                        <button @click.stop="chat.minimized = !chat.minimized" class="p-1 hover:bg-gray-100 rounded-full">
                            <span class="material-symbols-outlined text-[16px] text-gray-500">remove</span>
                        </button>
                        <button @click.stop="openChats = openChats.filter(c => c.id !== chat.id)" class="p-1 hover:bg-gray-100 rounded-full text-gray-500">
                ...
                <div x-show="!chat.minimized" 
                    ...
                    <template x-for="msg in chat.messages" :key="msg.id">
                        <div :class="msg.user_id === {{ Auth::id() }} ? 'bg-secondary text-slate-900 self-end' : 'bg-gray-100 text-gray-800 self-start'" 
                ...
                <div x-show="!chat.minimized" class="p-3 border-t bg-gray-50 flex items-center gap-2">
                    ...
                    " class="flex w-full gap-2">
                        <input type="text" x-model="chat.newMessage" placeholder="Écrire un message..." class="flex-1 text-sm px-4 py-2 bg-white rounded-full border border-gray-200 focus:outline-none focus:border-secondary focus:ring-1 focus:ring-secondary">
                        <button type="submit" class="text-secondary p-1 hover:bg-secondary/10 rounded-full">
    ...
    <div x-show="showMessenger" x-cloak class="fixed right-0 top-16 h-[calc(100vh-64px)] w-[260px] bg-slate-950 border-l border-slate-800 z-50 flex flex-col shadow-2xl">
        <div class="p-4 border-b border-slate-800">
            <h4 class="font-bold text-sm">Mes Étudiants</h4>
        </div>
        <div class="flex-1 overflow-y-auto">
            ...
            <button @click="
                ...
                <div class="w-8 h-8 rounded-full bg-secondary/20 flex items-center justify-center font-bold text-xs text-secondary">{{ substr($student->name, 0, 1) }}</div>
                <span class="text-sm font-semibold">{{ $student->name }}</span>
            </button>
            @empty
            <p class="p-4 text-xs text-outline text-center italic">Aucun étudiant ne vous suit.</p>
            @endforelse
        </div>
    </div>
    <!-- Sidebar -->
